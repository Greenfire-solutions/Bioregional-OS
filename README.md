...
| **Ledger** | An economy the commons defines for itself. You choose the unit and say what one of it *is*, in your own words; you choose whether anybody issues it or whether it is zero-sum mutual credit where balances always add to nothing and going below zero means the group is extending you credit; you choose the limit, who may issue, and what it is redeemable for. What you do not choose is the arithmetic — entries are append-only, corrections are reversals that leave both visible, balances are added up from the entries and never stored, and every movement is two legs summing to zero. Creating a unit, opening a pool or retiring one needs a **decided council decision**: an economy is something a community agrees, not a setting somebody switches on |
