...
if (verifyAt) {
  ...
  console.log(`\n  ${c.dim}To use it: stop the OS, then copy this file over data/commons.db${c.reset}\n`);
  process.exit(0);
}
...
info(`${human(statSync(target).size)} — one file, complete, safe to copy anywhere`);
if (integrity !== 'ok') { warn(`SQLite reports: ${integrity}`); process.exit(1); }
...
console.log(`\n  ${c.dim}${line()}${c.reset}`);
console.log(`  ${c.dim}Keep it somewhere that is not this computer. Check one with:${c.reset}`);
